// Code generated by brent.go.tmpl. DO NOT EDIT.
...
// logger may be nil
func (F32) Brent(a, b, tol float32, f func(float32) float32, logger *log.Logger) (float32, error) {
	type float = float32
	abs := math32.Abs
	it := 0
	// calculer f(a)
	...
	if fa*fb >= 0 {
		return math32.NaN(), errors.New("brent: f(a) f(b) >= 0")
	}
	...
	for fb != 0 && abs(b-a) > tol {
		...
		it++
		if it == 1000 {
			return math32.NaN(), fmt.Errorf("brent: it=%d", it)
		}
...
// logger may be nil
func (F32) Bissection(a, b, tol float32, f func(float32) float32, logger *log.Logger) (float32, error) {
	type float = float32
	abs, NaN := math32.Abs, math32.NaN()
	it := 0
	// calculer f(a)
	// calculer f(b)
	fa, fb := f(a), f(b)
...
// see https://en.wikipedia.org/wiki/Brent%27s_method
// logger may be nil
func (F64) Brent(a, b, tol float64, f func(float64) float64, logger *log.Logger) (float64, error) {
	type float = float64
	abs := math64.Abs
	it := 0
	// calculer f(a)
	...
	// si f(a) f(b) >= 0 alors sortie (erreur) fin si
	if fa*fb >= 0 {
		return math64.NaN(), errors.New("brent: f(a) f(b) >= 0")
	}
	// si |f(a)| < |f(b)| alors échanger (a,b) fin si
	if abs(fa) < abs(fb) {
		a, fa, b, fb = b, fb, a, fa
	...
	for fb != 0 && abs(b-a) > tol {
		...
		it++
		if it == 1000 {
			return math64.NaN(), fmt.Errorf("brent: it=%d", it)
		}
		//     si f(a) ≠ f(c) et f(b) ≠ f(c) alors
		//         s := a f ( b ) f ( c ) ( f ( a ) − f ( b ) ) ( f ( a ) − f ( c ) ) + b f ( a ) f ( c ) ( f ( b ) − f ( a ) ) ( f ( b ) − f ( c ) ) + c f ( a ) f ( b ) ( f ( c ) − f ( a ) ) ( f ( c ) − f ( b ) ) {\displaystyle s:={\frac {af(b)f(c)}{(f(a)-f(b))(f(a)-f(c))}}+{\frac {bf(a)f(c)}{(f(b)-f(a))(f(b)-f(c))}}+{\frac {cf(a)f(b)}{(f(c)-f(a))(f(c)-f(b))}}} s:={\frac {af(b)f(c)}{(f(a)-f(b))(f(a)-f(c))}}+{\frac {bf(a)f(c)}{(f(b)-f(a))(f(b)-f(c))}}+{\frac {cf(a)f(b)}{(f(c)-f(a))(f(c)-f(b))}} (interpolation quadratique inverse)
		//     sinon
...
// Bissection find zero of f using Bissection's method
// logger may be nil
func (F64) Bissection(a, b, tol float64, f func(float64) float64, logger *log.Logger) (float64, error) {
	type float = float64
	abs, NaN := math64.Abs, math64.NaN()
	it := 0
	// calculer f(a)
	// calculer f(b)
	fa, fb := f(a), f(b)
